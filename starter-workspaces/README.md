...
| File | Use case | Workspace name in Bond |
|---|---|---|
...
> strategic decisions, not just investments), and a better fit for Bond's
> "persistent specialists that hold context across sessions" design.

## What's in each export

Bond's workspace export bundle (the JSON file) includes:
...
your own provider keys in Bond's **Settings → Providers**.

## Import flow

1. Open Bond on your machine (see [`../docs/install-windows.md`](../docs/install-windows.md) / [`../docs/install-macos.md`](../docs/install-macos.md) if you're not installed yet)
2. **Settings → Workspaces → Import workspace**
3. Pick the `.json` file
4. Bond opens the new workspace, ready to use

If you've never run Bond before, complete the
...
sample decisions are deliberately concrete so you can see what real Bond
